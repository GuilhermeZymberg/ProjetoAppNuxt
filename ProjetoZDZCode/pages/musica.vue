<template>
  ...
  <br>
  <div>
    <image src="/joia.jpg" height="256"></image>
    <p class="text-h5">bololohaha</p>
  </div>
  <br>
  <br>
  <br>
  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
      <v-toolbar-title>musicas</v-toolbar-title>
      <v-spacer></v-spacer> 
        <v-dialog
          v-model="dialog"
          max-width="500px"
...
<script>
  export default {
    ...
    methods: {
      ...
      deleteItemConfirm () {
        this.musicas.splice(this.editedIndex, 1)
        this.closeDelete()
      },
      ...
      save () {
        if (this.editedIndex > -1) {
          Object.assign(this.musicas[this.editedIndex], this.editedItem)
        } else {
          this.musicas.push(this.editedItem)
        }
        this.close()
      },
    },
